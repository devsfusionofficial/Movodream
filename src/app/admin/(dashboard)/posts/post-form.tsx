        ...
        <div className="space-y-6">
          <section className="rounded-2xl border border-[#ebe5ed] bg-white p-6 shadow-[0_5px_18px_rgba(34,20,40,0.025)] sm:p-7">
            ...
            <FieldGroup className="gap-5 outline-none">
              ...
              <Field><FieldLabel htmlFor="slug" className="text-[13px] font-semibold text-[#403445]">URL slug</FieldLabel><Input id="slug" placeholder="Auto-generated from title if left blank" className={inputClass} {...register('slug')} /><FieldDescription className="text-xs text-[#998f9f]">Use lowercase words separated by hyphens for a clean, shareable URL.</FieldDescription><FieldError errors={[errors.slug]} /></Field>
        ...
        <div className="space-y-6 xl:sticky xl:top-6">
          <section className="rounded-2xl border border-[#ebe5ed] bg-white p-6 shadow-[0_5px_18px_rgba(34,20,40,0.025)]">
            ...
            <FieldGroup className="gap-5">
              ...
              <Field><FieldLabel htmlFor="authorId" className="text-[13px] font-semibold text-[#403445]">Author</FieldLabel><div className="relative"><UserRound className="pointer-events-none absolute left-3.5 top-1/2 z-10 h-4 w-4 -translate-y-1/2 text-[#a79ba9]" /><Select value={authorId || undefined} onValueChange={(v) => v && setValue('authorId', v, { shouldDirty: true })}><SelectTrigger id="authorId" className={`${inputClass} pl-10`}><span className={`flex flex-1 text-left ${selectedAuthor ? 'text-[#33283a]' : 'text-[#a79ba9]'}`}>{selectedAuthor?.name ?? 'Select author'}</span></SelectTrigger><SelectContent>{authors.map((author) => <SelectItem key={author._id} value={author._id}>{author.name}</SelectItem>)}</SelectContent></Select></div></Field>
            </FieldGroup>
          </section>
