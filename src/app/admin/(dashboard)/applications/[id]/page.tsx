...
export default async function ApplicationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="admin-resource-page mx-auto max-w-[1280px] w-full space-y-4 pb-6 outline-none">
      <Link
      ...
      <header className="flex flex-col gap-3 border-b border-[#eee9f0] pb-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <h1 className="text-2xl font-semibold tracking-[-0.05em] text-[#21182a] sm:text-3xl">{application.name}</h1>
            <p className="mt-1 text-xs text-[#887f8e]">Applied for {job?.title ?? 'Unknown role'}</p>
          </div>
        </div>
        <span className="w-fit rounded-full border border-[#eadfe8] bg-white px-3 py-1 text-[10px] font-bold uppercase tracking-[0.16em] text-[#887f8e]">
          Secure candidate file
        </span>
      </header>

      <div className="space-y-6 rounded-2xl border border-[#ebe6ee] bg-white p-5 sm:p-7 shadow-[0_5px_18px_rgba(34,20,40,0.025)]">
        <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2 text-sm">
          <div className="rounded-xl border border-[#f2edf4] bg-[#fdfbfd] p-3.5">
            <dt className="text-xs font-medium text-[#887f8e]">Email Address</dt>
            <dd className="mt-1 font-semibold text-[#21182a] break-all">{application.email}</dd>
          </div>
          <div className="rounded-xl border border-[#f2edf4] bg-[#fdfbfd] p-3.5">
            <dt className="text-xs font-medium text-[#887f8e]">Phone Number</dt>
            <dd className="mt-1 font-semibold text-[#21182a]">{application.phone || 'Not provided'}</dd>
          </div>
          {application.location && (
            <div className="rounded-xl border border-[#f2edf4] bg-[#fdfbfd] p-3.5">
              <dt className="text-xs font-medium text-[#887f8e]">Location</dt>
              <dd className="mt-1 font-semibold text-[#21182a]">{application.location}</dd>
            </div>
          )}
          {application.experience && (
            <div className="rounded-xl border border-[#f2edf4] bg-[#fdfbfd] p-3.5">
              <dt className="text-xs font-medium text-[#887f8e]">Experience</dt>
              <dd className="mt-1 font-semibold text-[#21182a]">{application.experience}</dd>
            </div>
          )}
          {application.qualification && (
            <div className="rounded-xl border border-[#f2edf4] bg-[#fdfbfd] p-3.5">
              <dt className="text-xs font-medium text-[#887f8e]">Qualification</dt>
              <dd className="mt-1 font-semibold text-[#21182a]">{application.qualification}</dd>
            </div>
          )}
          <div className="rounded-xl border border-[#f2edf4] bg-[#fdfbfd] p-3.5">
        ...
        {application.coverLetter && (
          <div className="rounded-xl border border-[#f2edf4] bg-[#fdfbfd] p-4">
            <h2 className="text-xs font-semibold text-[#887f8e]">Cover letter / note</h2>
            <p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-[#21182a]">{application.coverLetter}</p>
          </div>
        )}
