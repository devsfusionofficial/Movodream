      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-[272px] shrink-0 flex-col border-r border-[#eee9f0] bg-[#1b0d27] text-white shadow-2xl outline-none transition-transform duration-300 lg:sticky lg:top-0 lg:z-auto lg:h-screen lg:min-h-0 lg:translate-x-0 lg:self-start lg:shadow-none ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex h-16 items-center justify-between border-b border-white/10 px-7">
          <Link href="/admin" onClick={() => setOpen(false)} className="flex items-center gap-2.5">
            {/* Real brand lockup rather than a stand-in icon. The wordmark's
                "DREAM" half is dark purple, so it needs a lift to stay legible
                against the sidebar's near-black panel. */}
            <Image
              ...
              height={34}
              priority
              className="h-[26px] w-auto brightness-[1.35] contrast-[1.05]"
            />
            <span className="block border-l border-white/15 pl-2.5 text-[9px] font-medium uppercase leading-tight tracking-[0.2em] text-white/45">admin<br />workspace</span>
          </Link>
